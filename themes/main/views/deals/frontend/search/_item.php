<?php
/**
 * @author Bahdanovich Uladzimir <bahdanovich.uladzimir@gmail.com>
 * @date: 05.06.14
 */
$deal = $data;
;?>

<div class="panel panel-default">
    <div class="panel-body service-link cf">
        <div class="service-info">
            <a href="<?=$deal->getPublicUrl();?>" class="deal-name"><?=$deal->name;?></a>
            <a href="<?=$deal->user->getPublicUrl();?>" class="author-link"><?=$deal->user->getCommentUserName();?></a>
            <p class="ellipsis">
                <?php
                $this->widget('ext.XReadMore.XReadMore', array(
                    'showLink'=>false,
                    'model'=>$deal,
                    'attribute'=>'intro',
                    'maxChar'=>200,
                ));
                ?>
            </p>
            <div class="bottom-container">
                <div>
                    <?php $this->widget('modules.deals.widgets.addToFavorites.AddToFavoritesWidget', array(
                        'deal'=>$deal,
                    ));?>
                    <a href="<?=$deal->getPublicUrl();?>" class="more btn"><span class="a-spr"><?=Yii::t("dealsModule","More");?></span></a>
                </div>
                <div class="price">
                    <?php $this->widget('modules.deals.widgets.dealPrice.DealPriceWidget', array(
                        'deal'=>$deal,
                    ));?>
                </div>
                <?php $this->widget('modules.deals.widgets.dealRating.DealRatingWidget', array(
                    'deal'=>$deal,
                ));?>
            </div>
        </div>
        <?php if(sizeof($deal->frontendDealsImages)>0):?>
            <div class="gallery">
                <?php $counter=0;?>
                <?php foreach($deal->frontendDealsImages as $image):?>
                    <?php if($counter<3):?>
                        <div <?php echo ($counter==2) ? "class=\"hidden-sm\"": "";?>>
                            <?php echo CHtml::image($image->getSmallThumbUrl(),$deal->name);?>
                        </div>
                    <?php endif;?>
                    <?php $counter++;?>
                <?php endforeach;?>
            </div>
        <?php else:?>
            <div class="gallery not-available">
                <div>
                    <span><?=Yii::t("dealsModule","No images");?></span>
                </div>
            </div>
        <?php endif;?>
    </div>
</div>