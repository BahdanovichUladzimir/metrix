...
class UploadableFbAvatarBehavior extends CActiveRecordBehavior{
    /**
     * @var string название атрибута, хранящего в себе файл
     */
    public $avatarAttributeName='fb_avatar';
    ...
    public function afterSave($event){
        if(!is_null($this->owner->{$this->avatarAttributeName}) && is_string($this->owner->{$this->avatarAttributeName}) && (strlen($this->owner->{$this->avatarAttributeName})>0)){
            $file = $this->owner->{$this->avatarAttributeName};
            if($f=@fopen($file, 'rb')){
