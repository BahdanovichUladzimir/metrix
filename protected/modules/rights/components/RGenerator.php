...
class RGenerator extends CApplicationComponent
{
	...
	* @param string $aliasPath the path dir alias.
	* @return array the controllers.
	*/
	protected function getControllersInPath($path, $aliasPath = '')
	{
		$controllers = array();
			...
			foreach( $controllerDirectory as $entry )
			{
				if( $entry{0}!=='.' )
				{
					...
					if( strpos(strtolower($entry), 'controller')!==false )
					{
						...
						$controllers[ strtolower($entryPath) ] = array(
							'name'=>$name,
							'alias'=> ($aliasPath === '') ? $name : $aliasPath.".".$name,
							'file'=>$entry,
							'path'=>$entryPath,
						);
					}
					if( is_dir($entryPath)===true ){
						$aliasPath = ucfirst(basename($entryPath));
						foreach( $this->getControllersInPath($entryPath, $aliasPath) as $controllerName=>$controller ){
