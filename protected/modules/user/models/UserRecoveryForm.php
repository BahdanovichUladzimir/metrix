...
class UserRecoveryForm extends CFormModel {
	...
	public function rules()
	{
		return array(
			...
			array('login_or_email', 'match', 'pattern' => '/^[A-Za-z0-9@.-_\s,]+$/u','message' => Yii::t('userModule',"Incorrect symbols (A-z0-9).")),
	...
	{
		return array(
			'login_or_email'=>Yii::t('userModule',"username or email"),
		);
	}
	...
	public function checkexists($attribute,$params) {
		if(!$this->hasErrors())  // we only want to authenticate when no input errors
		{
			...
			if($user===null)
				if (strpos($this->login_or_email,"@")) {
					$this->addError("login_or_email",Yii::t('userModule',"Email is incorrect."));
				} else {
					$this->addError("login_or_email",Yii::t('userModule',"Username is incorrect."));
				}
		}
	}
