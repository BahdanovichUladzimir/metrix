...
class UserChangePassword extends CFormModel {
	...
	public function rules() {
		return Yii::app()->controller->id == 'recovery' ? array(
			...
			array('password, verifyPassword', 'length', 'max'=>128, 'min' => 4,'message' => Yii::t('userModule',"Incorrect password (minimal length 4 symbols).")),
			...
			array('oldPassword, password, verifyPassword', 'length', 'max'=>128, 'min' => 4,'message' => Yii::t('userModule',"Incorrect password (minimal length 4 symbols).")),
			array('verifyPassword', 'compare', 'compareAttribute'=>'password', 'message' => Yii::t('userModule',"Retype Password is incorrect.")),
	...
	{
		return array(
			'oldPassword'=>Yii::t('userModule',"Old Password"),
	 ...
	 */
	 public function verifyOldPassword($attribute, $params)
	 {
		 if (User::model()->notsafe()->findByPk(Yii::app()->user->id)->password != Yii::app()->getModule('user')->encrypting($this->$attribute))
			 $this->addError($attribute, Yii::t('userModule',"Old Password is incorrect."));
	 }
}