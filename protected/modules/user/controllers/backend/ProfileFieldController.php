...
class ProfileFieldController extends BackendController
{
	...
	var fieldType = {
			...
			'BOOL':{
				...
				'val':{
					'field_size':0,
					'default':0,
					'range':'1==".Yii::t('userModule','Yes').";0==".Yii::t('userModule','No')."',
					'widgetparams':''
				}
			},
			'BLOB':{
	...
	function showWidgetList(type) {
		...
		$('div.widget select').append('<option value=\"\">".Yii::t('userModule','No')."</option>');
		if (wgByType[type]) {
	...
	$('#dialog-form').dialog({
		...
		modal: true,
		buttons: {
			'".Yii::t('userModule','Save')."': function() {
				var wparam = {};
				var fparam = {};
			...
			'".Yii::t('userModule','Cancel')."': function() {
				$(this).dialog('close');
			}
		},
		close: function() {
	...
	// show all function 
	$('div.form p.note').append('<br/><a href=\"#\" id=\"showAll\">".Yii::t('userModule','Show all')."</a>');
	...
	public static function getWidgets($fieldType='') {
		...
		$widgets = array();
		$list = array(''=>Yii::t('userModule','No'));
		if (self::$_widgets) {
